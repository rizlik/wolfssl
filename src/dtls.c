...
static int ReadVector8(const byte* input, Vector* v)
{
    v->size = *input;
...
static int ReadVector16(const byte* input, Vector* v)
{
    word16 size16;
    ato16(input, &size16);
...
static int CreateDtlsCookie(WOLFSSL* ssl, const CH* ch, byte* cookie)
{
    Hmac cookieHmac;
    int ret;
...
static int ParseClientHello(const byte* input, word32 helloSz, CH* ch)
{
    word32 idx = 0;
...
static int TlsxFindByType(Vector* ret, word16 extType, Vector exts)
{
    word16 len, type;
    word16 idx = 0;
    Vector ext;
...
static int TlsTicketIsValid(WOLFSSL* ssl, Vector exts, byte *isValid)
{
    byte tempTicket[SESSION_TICKET_LEN];
    InternalTicket *it;
    Vector tlsxSessionTicket;
    int ret;
...
static int TlsResumptionIsValid(WOLFSSL* ssl, CH *ch, byte *isValid)
{
    int ret;
